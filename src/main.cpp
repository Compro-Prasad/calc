...
unsigned short record = NORMAL_COMMANDS | EXPRESSION_COMMANDS; /* Switch for (not)storing specific types commands */
      ...
      for (long i = 0;;)
	{
	  ...
	  if ((ch >= ' ' && ch <= DEL) || ch == LF)
	      ...
	      if (ch == DEL)
		{
	      ...
	      else if (i < strMAX - 5)	/* for other characters */
		{
